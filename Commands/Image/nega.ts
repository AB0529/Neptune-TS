...
class Command extends _Command {
	constructor(nep: Neptune) {
		...
		super(nep, {
			...
			help: `"Negative" image..`,
			longHelp: `Adds a negative effect to an image.`,
	...
	public async _run(msg: Message, args: string[], util: Util, nep: Neptune) {
		...
		const explode = async (link: string) => {
			...
			gm(bufLink).resize(512, 512).colorize(200, 200, 256).autoOrient().toBuffer(`jpg`, (err, buffer) => {
